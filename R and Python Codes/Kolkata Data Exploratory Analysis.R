...
#aqi_levels <- c("Good", "Satisfactory", "Moderate", "Poor", "Very Poor") # for Mumbai Data
...
ggplot(aqi_yearly, aes(x = factor(Year), y = AQI_Category, fill = ifelse(Percentage == 0, NA, Percentage))) +
  ...
  geom_text(aes(label = ifelse(Percentage == 0, "", paste0(round(Percentage, 0), "%"))), color = "black") +
...
aqi_heatmap <- aqi_heatmap %>% group_by(Month, AQI_Category) %>% 
  ...
  group_by(Month) %>% mutate(Percentage = Frequency * 100 / sum(Frequency))
ggplot(aqi_heatmap, aes(x = Month, y = AQI_Category, fill = Percentage)) + geom_tile(color = "white") +
  geom_text(aes(label = ifelse(Percentage == 0, "", paste0(round(Percentage, 0), "%"))), color = "black") +
  scale_fill_gradient(low = "lightblue", high = "darkred") +
  labs(title = NULL, x = "Month", y = "AQI Category", fill = "Percentage") + theme_minimal() +
