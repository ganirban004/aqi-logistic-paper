...
compute_metrics(actual = y.test, predicted = pred_MTD, levels = 0:4)
compute_metrics(actual = y.test, predicted = pred_PAR, levels = 0:4)
...
##### ML MODELS #####
dml <- d
dml$Category <- factor(dml$Category, levels = 0:4, ordered = T)
...
best_k$aic_table
tsolr_model <- tsolr_data(y = train_data$Response, best_K = c(2, 1), frequency = c(365, 7))
actual_tsolr <- as.numeric(test_data$Response) - 1
pred_tsolr <- predict_tsolr(model = tsolr_model, train_y = train_data$Response, 
                            test_y = test_data$Response, best_K = c(2, 1), 
                            frequency = c(365, 7))$predicted_class
pred_tsolr[1] <- 2

compute_metrics(actual_tsolr, pred_tsolr, levels = 0:4)



### ISOLR ###
isolr_model <- vglm(Response ~ Season + Festivity + Daytype + I0 + I1 + I2 + I3,
                    family = cumulative(link = "logitlink", parallel = ~ Season + Festivity + 
                                          Daytype + I0 + I1 + I2 + I3 - 1, reverse = FALSE),
                    data = train_data)
...
compute_metrics(actual_isolr, pred_isolr, levels = 0:4)
...
compute_metrics(actual_rf, pred_rf, levels = 0:4)



### Support Vector Machine ###
svm_model <- svm(Response ~ Season + Festivity + Daytype + Lag1, data = train_data, type = 'C', kernel = "sigmoid")
actual_svm <- as.numeric(test_data$Response) - 1
pred_svm <- as.numeric(predict(svm_model, test_data)) - 1

compute_metrics(actual_svm, pred_svm, levels = 0:4)
...
compute_metrics(actual_sarima, pred_sarima, levels = 0:4)
...
compute_metrics(actual_tbats, pred_tbats, levels = 0:4)
...
plot_log <- ggplot(plot_data, aes(x = Time)) + 
  ...
  geom_line(aes(y = TSOLR, colour = "TSOLR")) + coord_cartesian(ylim = c(0, 4)) + 
...
plot_markov <- ggplot(plot_data, aes(x = Time)) + 
  ...
  geom_line(aes(y = PAR, colour = "PAR")) + coord_cartesian(ylim = c(0, 4)) + 
...
plot_ml <- ggplot(plot_data, aes(x = Time)) + 
  ...
  geom_line(aes(y = SVM, colour = "SVM")) + coord_cartesian(ylim = c(0, 4)) + 
...
plot_dl <- ggplot(plot_data, aes(x = Time)) + 
  ...
  geom_line(aes(y = TFT, colour = "TFT")) + coord_cartesian(ylim = c(0, 4)) + 
...
plot_con <- ggplot(plot_data, aes(x = Time)) + 
  ...
  geom_line(aes(y = TBATS, colour = "TBATS")) + coord_cartesian(ylim = c(0, 4)) + 
